            ...
            <div class="row align-items-center">
                <div class="border-0 mb-4">
                    <div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
                        <h3 class="fw-bold mb-0">About Us Content</h3>
                        <div class="btn-set-task w-sm-100">
                            <button id="add_slider_btn" class="btn btn-primary py-2 px-5 w-sm-100"><i class="icofont-plus-circle me-2 fs-6"></i> Add About</button>
                        </div>
                    </div>
                </div>
            </div> <!-- Row end  -->
            ...
            <div class="row">
                <div class="col-12 border border-primary" id="add_slider_form" @if($errors->any()) style="display: block !important" @endif style="display: none">
                    <div class="slider-add-form-wrap py-2">
                        <form action="{{ route('about_us.add') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="slider-add-contents d-flex align-items-center justify-content-between" style="flex-flow: row wrap;">
                                <div class="item-wrp mb-2 w-100">
                                    <label for="sliderText" class="form-label">About Text</label>
                                    <textarea name="about_text" id="sliderText" placeholder="Write about your comapny" class="form-control"></textarea>
                                    @error('about_text')
                                        <div class="alert alert-warning mt-1">{!! $message !!}</div>
                                    @enderror
                                </div>
                                <div class="item-wrp mb-2 pe-4">
                                    <label for="sliderImage" class="form-label">About Image</label>
                                ...
                                <div class="item-wrp text-end">
                                    <button type="submit" class="btn btn-primary py-2 px-5 w-sm-100">Add About</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="single-slider col-xxl-3 col-md-4 col-sm-6 mb-3">
                        <div class="card">
                            <div class="img_box p-3">
                                <img class="card-img-top" src="{{ asset('dashboard_assets/images/about_us') }}/{{ $item->image }}" alt="About_us_{{ $key+1 }}" />
                            </div>
                            <div class="card-body">
                                <div class="title-del-btn d-flex justify-content-between align-items-center mb-3">
                                    <h5 class="card-title">About US {{ $key+1 }}</h5>
                                    <a href="{{ route('delete.about_us',encrypt($item->id)) }}" class="btn btn-outline-secondary deleterow slider-delete-btn opacity-0 invisible"><i class="icofont-ui-delete text-danger"></i></a>
                                </div>
                                <p class="card-text">{{ $item->about_text }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
